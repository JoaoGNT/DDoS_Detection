...
model_order = model_order = [(1, 1, 1), (2, 1, 1), (3, 1, 1), (1, 1, 2), (2, 1, 2), (3, 1, 2),
...
for n in range(0,len(model_order)-1):
    model = sm.tsa.arima.ARIMA(arimaData, order=model_order[n])
    ...
    for a in range(0,len(forecast)): #building a vector with the threshold
        tshUp = forecast[a] + 1.96 * math.sqrt(var)
        tshVecup.append(tshUp)
        tshDown = forecast[a] - 1.96 * math.sqrt(var)
    ...
    for i in range(0,len(result)):
        if (result[i][0] == 'True'):
            dt.append(df.loc[dv[i]])
            ns = ns+1
            window_atack.append(i)
        else:
            window_normal.append(i)
            nss = nss + 1
    # print('Ataque:', ns)
    # print('Normal:', nss)
    # print(window_atack)
    ...
    # print('Acertos:', acertos_normal+acertos_atack)
    # print('Erros:', erros_atack+erros_normal)
    # print('Linhas total:',acertos_normal+acertos_atack+erros_atack+erros_normal)
    # print('% acertos:',(acertos_normal+acertos_atack)*100/(acertos_normal+acertos_atack+erros_atack+erros_normal))
    ...
    #txt = "Modelo: {}\nAcertos: {}\nErros:{}\n% acertos: {}\nfalso_positivo{}\npositivo_falso{}".format(','.join([str(value) for value in model.order]),acertos,erros,porcacerto,erros_atack,erros_normal)
    # with open(','.join([str(value) for value in model.order]), 'w') as f:
    #     f.write(txt)

    vecacertos.append(acertos)
    vecerros.append(erros)
    vecfalso_positivo.append(erros_atack)
    vecpositivo_falso.append(erros_normal)
...
# d = {'Order': model_order, 'Acertos': vecacertos,'Erros':vecerros, 'Falso Positivo':vecfalso_positivo,
#      'Positivo Falso':vecpositivo_falso,'Taxa de Acerto':vectaxa_acerto}
